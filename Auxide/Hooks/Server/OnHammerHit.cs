...
namespace Auxide.Hooks.Server
{
    ...
    public static class OnHammerHit
    {
        static IEnumerable<CodeInstruction> Transpiler(IEnumerable<CodeInstruction> instr, ILGenerator il)
        {
            ...
            {
                if (codes[i].opcode == OpCodes.Ldstr && codes[i + 1].opcode == OpCodes.Ldc_I4_S && startIndex == -1)
                {
                    startIndex = i;
            ...
            if (startIndex > -1)
            {
                ...
                List<CodeInstruction> instructionsToInsert = new List<CodeInstruction>()
                {
                    ...
                    new CodeInstruction(OpCodes.Callvirt, AccessTools.Method(typeof(ScriptManager), "OnHammerHitHook")),
